...
int main(){
    ...
    disparoJugador(&jugador);
    for(int i = 0; i<balaAcutal; i++){
...
void disparoJugador(player * jugador){
...
void crearBala(player * jugador, int direccion){
    switch(direccion){
        case 2:
...
void handlerBalas(proyectile * bala, char mapa[fil][col]){
    ...
    printf("Bullet Position: X=%d, Y=%d, Direction=%d\n", bala->posX, bala->posY, bala->dir);
    if(bala->dir == 2){
