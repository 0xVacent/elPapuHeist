...
nodoArbol* agregaPuntuacion(nodoArbol* arbol,puntuacionJugador jugador,int puntaje)
{
    if(arbol)
    {
        arbol->izq=agregaPuntuacion(arbol->izq,jugador,puntaje);
        if(strcmp(arbol->dato.nombreJugador,jugador.nombreJugador)==0)
        {
            arbol->dato.puntaje[arbol->dato.validos]=puntaje;
            arbol->dato.validos++;
        }
        arbol->der=agregaPuntuacion(arbol->der,jugador,puntaje);
    }
